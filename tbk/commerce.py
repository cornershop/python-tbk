...
class Commerce(object):
    ...
    @classmethod
    def init_from_files(cls, commerce_code, key_file, cert_file, tbk_cert_file, environment):
        key_data = _read_file(key_file)
        cert_data = _read_file(cert_file)
        tbk_cert_data = _read_file(tbk_cert_file)
        return cls(commerce_code, key_data, cert_data, tbk_cert_data, environment)
